...
std::vector<char> getPerm2(const PMTree& tree, int num) {
    ...
    int n = static_cast<int>(tree.roots.size());

    if (static_cast<int64_t>(num) > factorial(n)) {
        return {};
    }
    ...
    for (int depth = n; depth > 0; --depth) {
        int64_t pps = factorial(depth - 1);
        int ci = static_cast<int>(idx / pps);
        idx %= pps;

        if (ci >= static_cast<int>(level->size())) {
            return {};
        }
